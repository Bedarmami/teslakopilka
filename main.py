...
keyboard.add(
    ...
    KeyboardButton("Ввести вручную"), KeyboardButton("Баланс"),
    KeyboardButton("Цель"), KeyboardButton("История"), KeyboardButton("Сбросить прогресс")
)
...
async def set_commands(bot: Bot):
    commands = [
        types.BotCommand(command="/start", description="Запустить копилку"),
        types.BotCommand(command="/balance", description="Показать баланс"),
        types.BotCommand(command="/reset", description="Сбросить накопления"),
...
@dp.message_handler(lambda message: message.text == "Ввести вручную")
async def handle_manual_input(message: types.Message):
    await message.reply("💬 Введите сумму вручную в формате +100 или -50.")

@dp.message_handler()
async def handle_custom_amount(message: types.Message):
    try:
        amount = int(message.text.replace(" зл", "").replace("+", "").strip())
        async with db_pool.acquire() as conn:
