...
async def get_history(conn, user_id):
    rows = await conn.fetch("SELECT total, last_update FROM balances WHERE user_id = $1", str(user_id))
    return rows
...
# Отчёт за неделю и месяц
async def send_weekly_report():
    async with db_pool.acquire() as conn:
        current_total = await get_balance(conn, ALLOWED_USER_ID)

        row = await conn.fetchrow("SELECT total, last_update FROM balances WHERE user_id = $1", str(ALLOWED_USER_ID))
        if not row:
            return
        ...
        row = await conn.fetchrow("SELECT total FROM balances WHERE user_id = $1", str(ALLOWED_USER_ID))
        past_week_date = datetime.utcnow() - timedelta(days=7)
        past_month_date = datetime.utcnow() - timedelta(days=30)

        weekly_gain = current_total - 0  # Упростим пока: раньше не сохранялась история
        monthly_gain = current_total - 0
